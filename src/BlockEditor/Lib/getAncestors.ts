...
export default function getAncestors(
    ...
    const trimmedBlockMap = revBlockMap
        .skipUntil((_, key) => key === targetKey)
        .skip(1)
    const parent = trimmedBlockMap
        .skipUntil(b => b.getDepth() < targetDepth)
        .first()
    return BlockMapBuilder.createFromArray([parent]).merge(
        getAncestors(trimmedBlockMap.reverse() as BlockMap, parent.getKey())
    )
}
