...
export default function setBlockAlignment(
    ...
    if (isSelectionInsideOneTable(editorState).isSelectionInsideOneTable) {
        ...
        const { cell: anchorCellN } = getOffsetPositionsInTable(
            selectionState.getAnchorOffset(),
            colN,
            text
        )
        const { cell: focusCellN } = getOffsetPositionsInTable(
            selectionState.getFocusOffset(),
            colN,
            text
        )
        const [startCellN, endCellN] = [
            Math.min(anchorCellN, focusCellN),
            Math.max(anchorCellN, focusCellN),
        ]

        const alignments = (() => {
            const alignments: Record<number, Alignment> =
                getObjData(currentData, 'alignments') || {}
            const resetAligns = _.chain(alignments)
                .thru(a =>
                    _.pickBy(a, (_, i: any) => i >= startCellN && i <= endCellN)
                )
        ...
        const newEditorState = mergeBlockData(editorState, blockKey, {
            alignments,
        })
